...
class MemberService {
  ...
  static async addMember(data) {
    ...
    await NotificationService.broadcastToProject(
      data.project_id,
      "MemberAdded",
      `A new member joined project ${data.project_id} with role ${data.role}`
    );
  ...
  static async updateMemberRole(id, role) {
    ...
    await NotificationService.broadcastToProject(
      member.project_id,
      "RoleChanged",
      `Member ${member.user_id}'s role changed to ${role}`
    );
  ...
  static async removeMember(id) {
    ...
    await NotificationService.broadcastToProject(
      member.project_id,
      "MemberRemoved",
      `A member was removed from project ${member.project_id}`
    );
